...
numbers = [int(s) for s in number_str.split(' ')]


# looking for 4 adjacent numbers
window = 4
rows = 20
cols = 20

max_product = 0
direction = None # right, up-right, down-right
position = None
...
def down(address):
    cols = 20
    return (address, address + cols, address + 2*cols, address + 3*cols)

def down_right(address):
    cols = 20
    return (address, address + cols + 1, address + 2*cols + 2, address + 3*cols + 3)

def up_right(address):
    cols = 20
    return (address, address - cols + 1, address - 2*cols + 2, address - 3*cols + 3)


#import pdb; pdb.set_trace()
rights, downs, up_rights, down_rights = [0] * 4
...
for row in range(0, rows):
    for col in range(0, cols):
        ...
        if col + window <= cols:
            rights += 1
            addresses = right(begin)
            product = reduce(mul, [numbers[a] for a in addresses])
        ...
        if row - window + 1>= 0 and col + window <= cols:
            up_rights += 1
            addresses = up_right(begin)
            product = reduce(mul, [numbers[a] for a in addresses])
            if product > max_product:
                max_product = product
                position = begin
